  ...
  return (
    ...
    >
      ...
      <motion.div style={{ y: blobY }} className="absolute inset-0 pointer-events-none">
        <FloatingShape
          className="absolute top-[10%] -left-[10%] w-[600px] h-[600px] rounded-full bg-blue/[0.07] blur-[120px] animate-mesh"
          delay={0}
        />
        <FloatingShape
          className="absolute top-[20%] right-[-5%] w-[500px] h-[500px] rounded-full bg-accent/[0.05] blur-[120px] animate-mesh-alt"
          delay={0.3}
        />
        <FloatingShape
          className="absolute bottom-[10%] left-[25%] w-[700px] h-[700px] rounded-full bg-gold/[0.03] blur-[140px] animate-mesh"
          delay={0.6}
        />
        <FloatingShape
          className="absolute -bottom-[15%] right-[20%] w-[400px] h-[400px] rounded-full bg-blue-light/[0.05] blur-[100px] animate-mesh-alt"
          delay={0.2}
        />
      </motion.div>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div
          ...
          className="absolute top-[15%] left-[10%] w-3 h-3 border border-blue/20 rotate-45"
        />
        <motion.div
          animate={{ y: [10, -10, 10], rotate: [360, 180, 0] }}
          transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
          className="absolute top-[25%] right-[15%] w-4 h-4 border border-gold/15 rounded-full"
        />
        <motion.div
          animate={{ y: [-15, 15, -15], x: [-5, 5, -5] }}
          transition={{ duration: 18, repeat: Infinity, ease: "linear" }}
          className="absolute bottom-[35%] left-[20%] w-2 h-2 bg-blue/20 rounded-full"
        />
        <motion.div
          animate={{ y: [5, -20, 5], rotate: [0, 90, 0] }}
          transition={{ duration: 12, repeat: Infinity, ease: "linear" }}
          className="absolute top-[40%] right-[8%] w-5 h-5 border border-accent/10 rotate-12"
        />
        <motion.div
          animate={{ y: [-8, 12, -8] }}
          transition={{ duration: 14, repeat: Infinity, ease: "linear" }}
          className="absolute bottom-[25%] right-[25%] w-2.5 h-2.5 bg-gold/15 rotate-45"
        />
        {/* Additional shapes for depth */}
        <motion.div
          animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.3, 0.1] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-[60%] left-[8%] w-20 h-20 border border-blue/5 rounded-full"
        />
        <motion.div
          animate={{ scale: [1.2, 1, 1.2], opacity: [0.05, 0.15, 0.05] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-[12%] right-[30%] w-32 h-32 border border-accent/5 rounded-full"
        />
      </div>
      ...
      >
        ...
        >
          <motion.div
            ...
            className="w-2 h-2 rounded-full bg-blue shadow-lg shadow-blue/50"
          />
          <span className="text-blue-light text-sm font-medium tracking-wide">
          ...
          {["London", "Singapore", "New York", "Dubai", "Hong Kong"].map((city, i) => (
            ...
            >
              <motion.span
                ...
                className="w-1 h-1 rounded-full bg-blue/50"
              />
              {city}
            </motion.span>
          ))}
